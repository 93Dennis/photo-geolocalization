...
from delg import image_reranking
...
def main(argv):
  ...
  for i in range(num_query_images):
    ...
    if FLAGS.use_geometric_verification:
      sorted_list_with_score = image_reranking.RerankByGeometricVerification(
    ...
    with open('%s/%s_result.txt' % (FLAGS.output_dir, query_list[i] ), 'w') as f:
      for item in sorted_list_with_score:
        image_name=index_list[item[0]]
        f.write("%s %s \n" % (item, image_name))  
